...
def animate_play(games,tracking_df,play_df,players,gameId,playId) -> go.Figure:
    ...
    for frameId in sorted_frame_list:
        ...
        # Plot the motion with a dashed line
        motion_man = None
        if "man_in_motion" in selected_tracking_df["event"]:
            motion_man = selected_tracking_df[selected_tracking_df["event"] == "man_in_motion"].sort_values(by="s", ascending=False).iloc[0]["nflId"]
        print("Man In Motion:", motion_man)
        # Plot Players
            ...
            if team != "football":
                ...
                data.append(
                    go.Scatter(
                        ...
                        # Make the color of the marker white if they are in motion, otherwise color_orders[team][0]
                        marker=[
                            go.scatter.Marker(color=color_orders[team][0], line=go.scatter.marker.Line(width=2,color=color_orders[team][1]),size=10) if motion_man is None else
                            go.scatter.Marker(color="white", line=go.scatter.marker.Line(width=2,color=color_orders[team][1]),size=10) for _ in range(len(plot_df))
                        ],
